...
const TimeReportContent = () => {
  ...
  const [employeeExpenseAmount, setEmployeeExpenseAmount] = useState();
...
/*const getExpensesAmountOnly = async () => {
    try {
      ...
      debugger;
      setEmployeeExpenseAmount(result.total_amount);
    ...
    <div>
      <div className="self-stretch flex flex-col items-start justify-start py-0 pl-0 box-border gap-[20px] max-w-full text-left text-xl text-black font-poppins">
        <div className="self-stretch flex flex-col items-start justify-start max-w-full text-sm border-box">
          ...
          <div className="flex status-flex py-2 px-3">
            <div className="expense-report-content">
              <p>Timesheet and Expenses Reported for: <span className="theme-txt px-2">
                {approvalEmployeeName ? approvalEmployeeName : localStorage.getItem("emp_UserName")}
              </span></p>
              <p>Hours submitted: <span className="grey-txt"> 
                {totalSum ? totalSum :0}/40
              </span>, Expenses submitted: <span className="grey-txt"> ₹ {employeeExpenseAmount ? employeeExpenseAmount : 0}</span></p>
            </div>
            <div className="status-title">
              Status:{" "}
              <span
                className={
                  status === "To Be Submitted" ? "tosubmit-clr" : status === "To Be Approved" ? "text-danger" : "text-success"
                }
              >
                {status}
              </span>
